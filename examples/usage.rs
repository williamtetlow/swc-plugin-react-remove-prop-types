...
fn get_abs_path_to_wasm_bin() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("target/wasm32-wasi/debug/swc_plugin_react_remove_prop_types.wasm")
}
...
fn main() {
    ...
    if !wasm_bin_path.exists() {
        panic!("wasm binary does not exist - you need to run cargo build before running example")
    }

    let opts = get_options(&wasm_bin_path);
    let code = r#"Greeting.propTypes = {
